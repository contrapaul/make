...
## What changed — bug-fix test gaps CLOSED (2026-09-04)

The bug-fix pass left two documented gaps: nothing exercised `initApp()`, and no
test pinned TTFT to an absolute magnitude. Both are now closed, and **both new
suites were validated by reintroducing the original defect and confirming they
fail** — a test that cannot fail is worth nothing.

| File | Change |
|---|---|
| `test/ui.test.mjs` | +4 checks, new **bootstrap smoke** section. `makeBootEl()` / `makeBootDoc()` build a full-document fixture with routable `.tab-panel` + `[data-tab-link]` nodes and a `documentElement` that records `setAttribute`; globals (`document`, `window`, `location`, `history`, `localStorage`, `matchMedia`, `IntersectionObserver`, `rAF`) are installed, then `js/app.js` is re-imported **cache-busted** so its auto-bootstrap branch really re-evaluates, then all globals are restored in a `finally`. Asserts the bootstrap does not throw **and** that it actually wired up: theme (`data-theme`), router (deep link `#/lab` activates only the Lab panel), tracker (`ui.visitedTabs === ['lab']`). `rAF` is a no-op so the Stage 3 drip never starts. **80/80 green** (was 76) |
| `test/engine.test.mjs` | +5 checks pinning TTFT magnitude: prefill throughput must land in 1e2–1e6 tok/s (derived, not a magic number — a 1e12 slip misses by twelve orders), default 8B rig TTFT in 0.1 s–60 s, and a sane-band sweep across GPU-resident / 70B offload-split / long-prompt so a unit slip on any branch is caught. **67/67 green** (was 62) |

**Validation:** moving `const defaultDoc`/`defaultHash` back to the end of `app.js` makes the UI suite fail with the original `ReferenceError: Cannot access 'defaultDoc' before initialization` (exit 1). Stripping the three `* 1e12` conversions from `perf.js` fails all 5 new engine checks — **while the pre-existing 62 all still pass**, which is exactly why that bug shipped. Both source files were restored byte-identical; only the two test files changed.

**Known latent issue found while building the fixture (NOT fixed, out of scope):** calling `initLab({ store })` a **second time** against the same store hangs the process synchronously — two Lab instances bound two-way to one store echo each other. Unreachable in production (the bootstrap guard runs once), so the fixture deliberately initialises once. Worth a guard in `lab.js` if double-init ever becomes reachable.
...
3. ~~Close the two open test gaps~~ — **DONE 2026-09-04** (`initApp()` bootstrap smoke + absolute TTFT band; both validated against the original defects). Remaining gap: the fixture asserts wiring, not rendering — visual correctness still needs a real browser. Previously shipped once.
...
| `test/ui.test.mjs` | ✅ **80 checks green** (was 70; +6 P5 M3 blocks: prefillDecodeView fast/slow, prefill-half engine prompt-token count, clock-driven drip 0→256, reduced-motion instant, store-change re-render); harness: `makeLabDoc()`, `makePrintout()`, `makeClock()`, `makePipeDoc()` (now incl. pipe-loadbar/-caption/-layers + pipe-prefill/-decode/-phase/-drip fakes) patterns in-file. ✅ **Both bug-fix test gaps closed 2026-09-04**: `initApp()` bootstrap smoke (cache-busted re-import against a full-document fixture) + absolute TTFT band in `engine.test.mjs`; each validated by reintroducing the original defect |
