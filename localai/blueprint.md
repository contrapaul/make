...
**Verification log (P7 M2, 2026-09-05):** the comparison table built, all ten §6 Tab 4 dimensions across five columns (the reader's machine plus four cloud models). **The acceptance line "every authoritative number has a footnote" shaped the data structure:** a cell is `{ text, sources }`, markers render as real `<sup class="fn">` elements rather than being baked into the text, and the source list is built from the sources actually cited. A test walks every numeric row and fails if a cloud number lacks a source id. **One number is deliberately withheld:** DeepSeek V4 Pro's SWE-bench highlight in cloud.js carries P1's own instruction "third-party aggregation; verify before publishing", so `agenticHighlight()` filters it out rather than putting an unverified third-party figure on a page whose promise is that every number is sourced; the cell says so plainly, and a test pins the behaviour. The same selector also stops GPT-5.6 Sol's knowledge cutoff being printed under "Agentic coding", which taking `highlights[0]` would have done. Only Claude Opus 5 has a genuine uncaveated coding claim, which is an honest picture of the sources. **Cost row:** local ¥1.10 per million output tokens against ¥8.87 / ¥26.61 / ¥168.00 / ¥201.60 for the cloud tiers, with the table's notes stating that the two are different kinds of cost (electricity plus amortised hardware against a published token price at 6.72). Pricing caveats are generated from cloud.js by `pricingCaveats()` rather than retyped, so the DeepSeek peak/off-peak split and the GPT-5.6 promotional pricing reach the reader. The local column states plainly that answer quality is not measured by this site. **Also fixed:** headers read "DeepSeek DeepSeek V4 Pro" because the vendor is stored separately from a name that already carries it; `displayName()` deduplicates and a test asserts no header repeats a word. **Tests:** `test/ui.test.mjs` **133/133 green** (was 125); `test/engine.test.mjs` 63/63 unchanged. Live check: table renders 10 rows and 5 columns with 20 footnote markers resolving to 8 listed sources, 9 reading notes, console clean.
...
| **P7 Compare tab** | Tab 4 complete — ✅ M1 race · ✅ M2 comparison table (both built 2026-09-05) · M3 cost panel still to build | Race ≤20 s; footnotes on all authoritative numbers; cost math verified |
